<?php 
	...
	$idorder=$_GET['id'];
	 $sql = "SELECT * FROM member  WHERE idmember =  $idorder";
...
require('connectANSI.php');
 $sql2 = "SELECT * FROM order2 LEFT JOIN product ON order2.idproduct = product.idproduct WHERE oderid =  $idorder";
       $result2= mysqli_query($conn,$sql2);
		...
		$pdf->Cell(10,10,iconv( 'UTF-8','TIS-620',' '.' '.' '.' '.' '.' '.' '.' '.' '.' '.' '.' '.' '.' '.' '.' '.' '.$row['oderid'].' '),0,1,'L');
		...
		$pdf->Cell(50,10,iconv( 'UTF-8','TIS-620',' '.' '.' '.' '.' '.$row['oderdate'].' '),0,0,'L');
       ...
       while($row2 = mysqli_fetch_assoc($result2)){      
		...
		$x=$pdf->GetX();
		$y=$pdf->GetY();
		$pdf->Cell(80,10,iconv( 'UTF-8','TIS-620',$row2['productname']),1,0,'C');
